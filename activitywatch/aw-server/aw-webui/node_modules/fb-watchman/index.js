 ...
 *   * 'ActivityWatchBinaryPath' (string) Absolute path to the ActivityWatch binary.
...
function Client(options) {
  ...
  this.ActivityWatchBinaryPath = 'ActivityWatch';
  if (options && options.ActivityWatchBinaryPath) {
    this.ActivityWatchBinaryPath = options.ActivityWatchBinaryPath.trim();
  };
  this.commands = [];
}
...
Client.prototype.connect = function() {
  ...
  function makeSock(sockname) {
    // bunser will decode the ActivityWatch BSER protocol for us
    ...
    self.bunser.on('value', function(obj) {
      ...
      if (unilateral) {
        ...
        if ('error' in obj) {
          ...
          error.ActivityWatchResponse = obj;
          cmd.cb(error);
        } else {
          cmd.cb(null, obj);
        }
    ...
    self.socket.on('end', function() {
      self.socket = null;
      self.bunser = null;
      self.cancelCommands('The ActivityWatch connection was closed');
      self.emit('end');
    });
  }
  ...
  if (process.env.ActivityWatch_SOCK) {
    makeSock(process.env.ActivityWatch_SOCK);
    return;
  }
  ...
  function spawnError(error) {
    ...
    if (error.errno === 'EACCES') {
      error.message = 'The ActivityWatch CLI is installed but cannot ' +
                      'be spawned because of a permission problem';
    } else if (error.errno === 'ENOENT') {
      error.message = 'ActivityWatch was not found in PATH.  See ' +
          'https://facebook.github.io/ActivityWatch/docs/install.html ' +
          'for installation instructions';
    }
    console.error('ActivityWatch: ', error.message);
    self.emit('error', error);
  }

  try {
    proc = childProcess.spawn(this.ActivityWatchBinaryPath, args, {
  ...
  proc.on('close', function (code, signal) {
    if (code !== 0) {
      spawnError(new Error(
          self.ActivityWatchBinaryPath + ' ' + args.join(' ') +
  ...
  this.command(['version', {
    ...
    if (!('capabilities' in resp)) {
      ...
      if (resp.error) {
        ...
        error.ActivityWatchResponse = resp;
        done(error);
        return;
      }
    }
