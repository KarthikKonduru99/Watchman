...
var _ActivityWatch = _interopRequireDefault(require('./crawlers/ActivityWatch'));
...
var _ActivityWatchWatcher = _interopRequireDefault(
  require('./watchers/ActivityWatchWatcher')
);
...
const canUseActivityWatch = (() => {
  try {
    (0, _child_process().execSync)('ActivityWatch --version', {
      stdio: ['ignore']
    });
    return true;
  } catch {}
 ...
 *
 * type HasteMap = {
 *   clocks: ActivityWatchClocks,
 *   files: {[filepath: string]: FileMetaData},
 *   map: {[id: string]: ModuleMapItem},
 *   mocks: {[id: string]: string},
 * }
 *
 * // ActivityWatch clocks are used for query synchronization and file system deltas.
 * type ActivityWatchClocks = {[filepath: string]: string};
 *
 * type FileMetaData = {
 ...
 *     * cache available:
 *       * if ActivityWatch is available: get file system delta changes.
 *       * if ActivityWatch is unavailable: crawl the entire file system.
...
class HasteMap extends _events().EventEmitter {
  ...
  constructor(options) {
    ...
    this._options = {
      ...
      useActivityWatch: options.useActivityWatch == null ? true : options.useActivityWatch,
      watch: !!options.watch
    };
    ...
    if (this._options.enableSymlinks && this._options.useActivityWatch) {
      throw new Error(
        'jest-haste-map: enableSymlinks config option was set, but ' +
          'is incompatible with ActivityWatch.\n' +
          'Set either `enableSymlinks` to false or `useActivityWatch` to false.'
      );
    }
  ...
  _crawl(hasteMap) {
    ...
    const crawl =
      canUseActivityWatch && this._options.useActivityWatch
        ? _ActivityWatch.default
        : _node.default;
    ...
    const retry = error => {
      if (crawl === _ActivityWatch.default) {
        this._console.warn(
          'jest-haste-map: ActivityWatch crawl failed. Retrying once with node ' +
            'crawler.\n' +
            "  Usually this happens when ActivityWatch isn't running. Create an " +
            "empty `.ActivityWatchconfig` file in your project's root folder or " +
  ...
  _watch(hasteMap) {
    ...
    this._options.retainAllFiles = true; // ActivityWatchWatcher > FSEventsWatcher > sane.NodeWatcher

    const Watcher =
      canUseActivityWatch && this._options.useActivityWatch
        ? _ActivityWatchWatcher.default
