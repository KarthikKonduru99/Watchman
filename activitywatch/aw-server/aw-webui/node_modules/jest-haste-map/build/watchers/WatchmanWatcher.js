...
exports.default = ActivityWatchWatcher;
...
function ActivityWatchWatcher(dir, opts) {
...
ActivityWatchWatcher.prototype.__proto__ = _events().EventEmitter.prototype;
/**
 * Run the ActivityWatch `watch` command on the root and subscribe to changes.
 *
 * @private
 */

ActivityWatchWatcher.prototype.init = function () {
  if (this.client) {
    this.client.removeAllListeners();
  }

  const self = this;
  this.client = new (_fbActivityWatch().default.Client)();
  this.client.on('error', error => {
    self.emit('error', error);
  });
  this.client.on('subscription', this.handleChangeEvent.bind(this));
  this.client.on('end', () => {
    console.warn('[sane] Warning: Lost connection to ActivityWatch, reconnecting..');
    self.init();
  });
  ...
  function onCapability(error, resp) {
    if (handleError(self, error)) {
      // The ActivityWatch watcher is unusable on this system, we cannot continue
      return;
    }
  ...
  function onClock(error, resp) {
    ...
    // to the ActivityWatch server.  This saves both on data size to be
...
ActivityWatchWatcher.prototype.handleChangeEvent = function (resp) {
...
ActivityWatchWatcher.prototype.handleFileChange = function (changeDescriptor) {
  const self = this;
  let absPath;
  let relativePath;
...
ActivityWatchWatcher.prototype.emitEvent = function (
  eventType,
  filepath,
  root,
  stat
...
ActivityWatchWatcher.prototype.close = function () {
  this.client.removeAllListeners();
  this.client.end();
 ...
 *
 * @param {ActivityWatchWatcher} self
 * @param {Error} error
 * @private
 */
...
/**
 * Handles a warning in the ActivityWatch resp object.
 *
 * @param {object} resp
 * @private
 */
