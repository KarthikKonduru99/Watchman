...
from aw_core.log import setup_logging, dirs
...
def main():
    ...
    try:
        paths = ['aw-qt', 'aw-server', 'aw-watcher-afk', 'aw-watcher-window']
        for path in paths:
            path = dirs.get_log_dir(path)
            for file in os.listdir(path):
                creation_date = os.stat(os.path.join(path, file)).st_ctime
                if creation_date < time.time() - 7 * 86400:
                    os.remove(os.path.join(path, file))
        logger.info("Files older than 7 days has been deleted")
    except Exception as e:
        logger.error("Cannot delete files due to {}".format(e))
